...
RU_TEXT = {
    ...
    "Status / Note": "Статус / примечание (Status / Note)",
    "Formula / meaning": "Формула / смысл (Formula / meaning)",
    "Result": "Результат (Result)",
    "Item": "Показатель (Item)",
    "Source": "Источник (Source)",
    "Reason": "Причина (Reason)",
    "Status": "Статус (Status)",
    "Next action": "Следующее действие (Next action)",
    "Check": "Проверка (Check)",
    "Inverter model": "Модель инвертора (Inverter model)",
    "Panel model": "Модель панели (Panel model)",
    "Battery model": "Модель АКБ (Battery model)",
    "Battery quantity": "Количество АКБ (Battery quantity)",
    "Minimum ambient temperature": "Минимальная температура воздуха (Minimum ambient temperature)",
    "Maximum cell temperature": "Максимальная температура ячейки (Maximum cell temperature)",
    "Panels per string": "Панелей в строке (Panels per string)",
    "Strings per MPPT": "Строк на MPPT (Strings per MPPT)",
    "Roof type": "Тип кровли (Roof type)",
    "Total panels": "Всего панелей (Total panels)",
    "Panels per row": "Панелей в ряду (Panels per row)",
    "Rail stock length": "Длина рейки со склада (Rail stock length)",
    "Rail length per panel": "Длина рейки на панель (Rail length per panel)",
    "Mounting reserve": "Запас крепежа (Mounting reserve)",
    "DC cable route length": "Длина DC кабеля (DC cable route length)",
    "AC cable route length": "Длина AC кабеля (AC cable route length)",
    "Grounding cable route length": "Длина кабеля заземления (Grounding cable route length)",
    "Installation method": "Способ прокладки (Installation method)",
    "Cable grouping derating": "Коэффициент группировки кабелей (Cable grouping derating)",
    "Ambient temperature derating": "Коэффициент температуры окружающей среды (Ambient temperature derating)",
    "Lightning protection system": "Система молниезащиты (Lightning protection system)",
    "AC earthing system": "Система заземления AC (AC earthing system)",
    "Region / tariff zone": "Регион / тарифная зона (Region / tariff zone)",
    "Self-consumption share": "Доля собственного потребления (Self-consumption share)",
    "Installed system cost": "Стоимость установленной системы (Installed system cost)",
    "Tariff source status": "Статус источника тарифа (Tariff source status)",
    "Monthly consumption": "Потребление в месяц (Monthly consumption)",
    "Target coverage": "Целевое покрытие (Target coverage)",
    "Tariff mode": "Режим тарифа (Tariff mode)",
    "Day consumption share": "Доля дневного потребления (Day consumption share)",
    "Day tariff": "Дневной тариф (Day tariff)",
    "Night tariff": "Ночной тариф (Night tariff)",
    "Design note": "Примечание по расчету (Design note)",
    "Choose from dropdown": "Выберите из списка (Choose from dropdown)",
    "Used for cold Voc check": "Для проверки Voc на холоде (Used for cold Voc check)",
    "Used for hot Vmp check": "Для проверки Vmp на нагреве (Used for hot Vmp check)",
    "Series modules in one string": "Модули последовательно в одной строке (Series modules in one string)",
    "Parallel strings on one tracker": "Параллельные строки на один трекер (Parallel strings on one tracker)",
    "Used for total battery capacity": "Для расчета общей емкости АКБ (Used for total battery capacity)",
    "Used for mounting quantity calculation": "Для расчета количества крепежа (Used for mounting quantity calculation)",
    "Used for row and clamp calculation": "Для расчета рядов и прижимов (Used for row and clamp calculation)",
    "Used for rail connector estimate": "Для оценки соединителей рейки (Used for rail connector estimate)",
    "Starter assumption; adjust by panel orientation": "Стартовое допущение; уточнить по ориентации панели (Starter assumption; adjust by panel orientation)",
    "Extra quantity reserve": "Дополнительный запас количества (Extra quantity reserve)",
    "One-way route length estimate": "Оценка длины трассы в одну сторону (One-way route length estimate)",
    "Used for cable derating notes": "Для примечаний по снижению нагрузки кабеля (Used for cable derating notes)",
    "Starter factor; adjust by local rules": "Стартовый коэффициент; уточнить по местным нормам (Starter factor; adjust by local rules)",
    "Used for SPD type suggestion": "Для рекомендации типа УЗИП (Used for SPD type suggestion)",
    "Used for earthing checklist": "Для чек-листа заземления (Used for earthing checklist)",
    "Choose from tariff database": "Выберите из базы тарифов (Choose from tariff database)",
    "Share of generation consumed on site": "Доля выработки, потребляемая на объекте (Share of generation consumed on site)",
    "Used for simple payback estimate": "Для оценки простой окупаемости (Used for simple payback estimate)",
    "Tariffs must be checked for exact supplier/date": "Тарифы нужно сверить по поставщику и дате (Tariffs must be checked for exact supplier/date)",
    "Customer monthly electricity consumption": "Месячное потребление клиента (Customer monthly electricity consumption)",
    "Recommended options cover this share of annual consumption": "Рекомендуемые варианты покрывают эту долю годового потребления (Recommended options cover this share of annual consumption)",
    "Single-rate or Day-night comparison": "Сравнение единого тарифа и день-ночь (Single-rate or Day-night comparison)",
    "Used for day-night tariff comparison": "Для сравнения тарифа день-ночь (Used for day-night tariff comparison)",
    "Starter day tariff; verify supplier/date": "Стартовый дневной тариф; сверить поставщика/дату (Starter day tariff; verify supplier/date)",
    "Starter night tariff; verify supplier/date": "Стартовый ночной тариф; сверить поставщика/дату (Starter night tariff; verify supplier/date)",
    "Starter calculation only": "Только стартовый расчет (Starter calculation only)",
    "Verify datasheets and mounting manuals before commercial use": "Проверить datasheet и инструкции по крепежу перед коммерческим применением (Verify datasheets and mounting manuals before commercial use)",
    "From Inputs": "Из листа Inputs (From Inputs)",
    "From Results": "Из листа Results (From Results)",
    "From Compatibility": "Из листа Compatibility (From Compatibility)",
    "From Batteries database": "Из базы Batteries (From Batteries database)",
    "From Panels database": "Из базы Panels (From Panels database)",
    "From Regions database": "Из базы Regions (From Regions database)",
    "From Tariffs database": "Из базы Tariffs (From Tariffs database)",
    "Selected model": "Выбранная модель (Selected model)",
    "Selected value": "Выбранное значение (Selected value)",
    "Selected roof": "Выбранная кровля (Selected roof)",
    "Nominal energy": "Номинальная энергия (Nominal energy)",
    "Overall compatibility": "Общая совместимость (Overall compatibility)",
    "PV electrical status": "Электрический статус PV (PV electrical status)",
    "Battery status": "Статус АКБ (Battery status)",
    "Datasheet completeness": "Полнота datasheet (Datasheet completeness)",
    "Inverter": "Инвертор (Inverter)",
    "Panel": "Панель (Panel)",
    "Battery": "АКБ (Battery)",
    "Choose inverter model": "Выберите модель инвертора (Choose inverter model)",
    "Choose panel model": "Выберите модель панели (Choose panel model)",
    "Choose battery model": "Выберите модель АКБ (Choose battery model)",
    "Choose roof type or ground mount": "Выберите тип кровли или наземную установку (Choose roof type or ground mount)",
    "Choose cable installation method": "Выберите способ прокладки кабеля (Choose cable installation method)",
    "Choose lightning protection condition": "Выберите условие молниезащиты (Choose lightning protection condition)",
    "Choose AC earthing system": "Выберите систему заземления AC (Choose AC earthing system)",
    "Choose tariff region": "Выберите тарифный регион (Choose tariff region)",
    "Choose tariff comparison mode": "Выберите режим сравнения тарифа (Choose tariff comparison mode)",
    "Recommended string range": "Рекомендуемый диапазон строки (Recommended string range)",
    "Battery total energy": "Общая энергия АКБ (Battery total energy)",
    "Rows": "Ряды (Rows)",
    "Rails": "Рейки (Rails)",
    "End clamps": "Крайние прижимы (End clamps)",
    "Middle clamps": "Средние прижимы (Middle clamps)",
    "Grounding clips": "Клипсы заземления (Grounding clips)",
    "Cable clips": "Кабельные клипсы (Cable clips)",
    "Equipment list items": "Позиций оборудования (Equipment list items)",
    "Protection items": "Позиций защиты (Protection items)",
    "DC cable section": "Сечение DC кабеля (DC cable section)",
    "AC cable section": "Сечение AC кабеля (AC cable section)",
    "Battery cable section": "Сечение кабеля АКБ (Battery cable section)",
    "SPD recommendation": "Рекомендация УЗИП (SPD recommendation)",
    "Earthing system": "Система заземления (Earthing system)",
    "Cable derating factor": "Коэффициент снижения нагрузки кабеля (Cable derating factor)",
    "Bulk scenarios": "Массовые сценарии (Bulk scenarios)",
    "Bulk PASS": "Массовые PASS (Bulk PASS)",
    "Bulk FAIL": "Массовые FAIL (Bulk FAIL)",
    "Bulk VERIFY": "Массовые VERIFY (Bulk VERIFY)",
    "Region": "Регион (Region)",
    "Annual generation": "Годовая выработка (Annual generation)",
    "Annual savings": "Годовая экономия (Annual savings)",
    "Simple payback": "Простая окупаемость (Simple payback)",
    "Recommended standard size": "Рекомендуемая стандартная мощность (Recommended standard size)",
    "Standard option annual generation": "Годовая выработка стандартного варианта (Standard option annual generation)",
    "Standard option payback day-night": "Окупаемость стандартного варианта день-ночь (Standard option payback day-night)",
    "Best month generation": "Выработка в лучший месяц (Best month generation)",
    "Worst month generation": "Выработка в худший месяц (Worst month generation)",
    "Monthly self-consumption value": "Ценность собственного потребления по месяцам (Monthly self-consumption value)",
    "Monthly export value": "Ценность экспорта по месяцам (Monthly export value)",
    "Regional yield estimate": "Оценка региональной выработки (Regional yield estimate)",
    "Self-consumption plus export": "Собственное потребление плюс экспорт (Self-consumption plus export)",
    "Installed cost / annual savings": "Стоимость системы / годовая экономия (Installed cost / annual savings)",
    "Must be verified": "Нужно проверить (Must be verified)",
    "Customer input": "Ввод клиента (Customer input)",
    "Option sizing target": "Цель подбора варианта (Option sizing target)",
    "SystemOptions": "Лист SystemOptions (SystemOptions)",
    "Mounting calculation": "Расчет крепежа (Mounting calculation)",
    "Mounting input": "Ввод для крепежа (Mounting input)",
    "Generated equipment output": "Сформированный список оборудования (Generated equipment output)",
    "Generated protection output": "Сформированный список защиты (Generated protection output)",
    "Starter sizing": "Стартовый подбор (Starter sizing)",
    "Starter protection selection": "Стартовый выбор защиты (Starter protection selection)",
    "Selected AC earthing system": "Выбранная система заземления AC (Selected AC earthing system)",
    "Grouping x ambient factors": "Группировка x температурные коэффициенты (Grouping x ambient factors)",
    "Mounting quantity": "Количество крепежа (Mounting quantity)",
    "Rail lines": "Линии реек (Rail lines)",
    "Estimated rail length": "Оценочная длина реек (Estimated rail length)",
    "Rail joints": "Стыки реек (Rail joints)",
    "Reserve": "Запас (Reserve)",
    "Component": "Компонент (Component)",
    "Quantity": "Количество (Quantity)",
    "Roof hooks / seam clamps / ground posts": "Крюки / фальцевые зажимы / наземные стойки (Roof hooks / seam clamps / ground posts)",
    "Mini-rails": "Мини-рельсы (Mini-rails)",
    "Rail connectors": "Соединители реек (Rail connectors)",
    "Bolt sets": "Комплекты болтов (Bolt sets)",
    "Two rail lines per row": "Две линии реек на ряд (Two rail lines per row)",
    "Total panels x rail length per panel x 2": "Всего панелей x длина рейки на панель x 2 (Total panels x rail length per panel x 2)",
    "Estimated from rail length and stock length": "Оценка по длине реек и складской длине (Estimated from rail length and stock length)",
    "Hooks, seam clamps, or ground posts depending on installation type": "Крюки, фальцевые зажимы или наземные стойки по типу установки (Hooks, seam clamps, or ground posts depending on installation type)",
    "Used for mini-rail roof systems": "Для кровельных систем с мини-рельсами (Used for mini-rail roof systems)",
    "Rail stock pieces": "Складские отрезки реек (Rail stock pieces)",
    "Usually two per rail joint": "Обычно два на стык рейки (Usually two per rail joint)",
    "Four per row": "Четыре на ряд (Four per row)",
    "Two per gap between panels in a row": "Два на промежуток между панелями в ряду (Two per gap between panels in a row)",
    "Fastener allowance": "Запас крепежа (Fastener allowance)",
    "One per panel starter assumption": "Стартово один на панель (One per panel starter assumption)",
    "Two per panel starter assumption": "Стартово два на панель (Two per panel starter assumption)",
    "Economics": "Экономика (Economics)",
    "Selected panel power": "Мощность выбранной панели (Selected panel power)",
    "PV system size": "Мощность СЭС (PV system size)",
    "Specific yield": "Удельная выработка (Specific yield)",
    "Performance ratio": "Коэффициент производительности (Performance ratio)",
    "Retail tariff": "Розничный тариф (Retail tariff)",
    "Export / green tariff": "Экспорт / зеленый тариф (Export / green tariff)",
    "Retail tariff source": "Источник розничного тарифа (Retail tariff source)",
    "Export tariff source": "Источник экспортного тарифа (Export tariff source)",
    "Annual consumption": "Годовое потребление (Annual consumption)",
    "Target annual coverage": "Целевое годовое покрытие (Target annual coverage)",
    "Day-night blended tariff": "Смешанный тариф день-ночь (Day-night blended tariff)",
    "Estimated current annual cost": "Оценочная текущая годовая стоимость (Estimated current annual cost)",
    "Estimated day-night annual cost": "Оценочная годовая стоимость день-ночь (Estimated day-night annual cost)",
    "Potential day-night saving before PV": "Потенциальная экономия день-ночь до СЭС (Potential day-night saving before PV)",
    "PV size x yield x performance ratio": "Мощность PV x выработка x коэффициент производительности (PV size x yield x performance ratio)",
    "Starter system loss factor": "Стартовый коэффициент потерь системы (Starter system loss factor)",
    "Starter microgeneration purchase value": "Стартовая цена покупки микрогенерации (Starter microgeneration purchase value)",
    "Source URL": "URL источника (Source URL)",
    "Monthly consumption x 12": "Месячное потребление x 12 (Monthly consumption x 12)",
    "Annual consumption x target coverage": "Годовое потребление x целевое покрытие (Annual consumption x target coverage)",
    "Day share x day tariff plus night share x night tariff": "Доля дня x дневной тариф плюс доля ночи x ночной тариф (Day share x day tariff plus night share x night tariff)",
    "Annual consumption x retail tariff": "Годовое потребление x розничный тариф (Annual consumption x retail tariff)",
    "Annual consumption x blended day-night tariff": "Годовое потребление x смешанный тариф день-ночь (Annual consumption x blended day-night tariff)",
    "Current cost minus day-night cost": "Текущая стоимость минус стоимость день-ночь (Current cost minus day-night cost)",
    "Option": "Вариант (Option)",
    "Quality": "Качество (Quality)",
    "Cost per kWp": "Стоимость за кВтп (Cost per kWp)",
    "Service life": "Срок службы (Service life)",
    "Degradation": "Деградация (Degradation)",
    "Required annual generation": "Требуемая годовая выработка (Required annual generation)",
    "Required kWp": "Требуемая мощность кВтп (Required kWp)",
    "Recommended kWp": "Рекомендуемая мощность кВтп (Recommended kWp)",
    "Panels": "Панели (Panels)",
    "Daily generation": "Дневная выработка (Daily generation)",
    "Monthly generation": "Месячная выработка (Monthly generation)",
    "Coverage": "Покрытие (Coverage)",
    "Estimated cost": "Оценочная стоимость (Estimated cost)",
    "Single-rate savings": "Экономия на едином тарифе (Single-rate savings)",
    "Day-night savings": "Экономия день-ночь (Day-night savings)",
    "Green tariff export revenue": "Выручка от зеленого тарифа (Green tariff export revenue)",
    "Payback day-night": "Окупаемость день-ночь (Payback day-night)",
    "Higher allowance; verify commercial offer": "Повышенный бюджет; сверить коммерческое предложение (Higher allowance; verify commercial offer)",
    "Balanced default option": "Сбалансированный вариант по умолчанию (Balanced default option)",
    "Lower cost allowance; verify warranties": "Пониженный бюджет; проверить гарантии (Lower cost allowance; verify warranties)",
    "Year": "Год (Year)",
    "Premium generation": "Выработка Premium (Premium generation)",
    "Standard generation": "Выработка Standard (Standard generation)",
    "Economy generation": "Выработка Economy (Economy generation)",
    "Premium cumulative": "Накопительно Premium (Premium cumulative)",
    "Standard cumulative": "Накопительно Standard (Standard cumulative)",
    "Economy cumulative": "Накопительно Economy (Economy cumulative)",
    "Month": "Месяц (Month)",
    "Generation": "Выработка (Generation)",
    "Consumption": "Потребление (Consumption)",
    "Self-consumed": "Собственное потребление (Self-consumed)",
    "Self-consumption value": "Стоимость собственного потребления (Self-consumption value)",
    "Exported": "Экспортировано (Exported)",
    "Export value": "Стоимость экспорта (Export value)",
    "Total value": "Общая стоимость (Total value)",
    "Jan": "Янв (Jan)",
    "Feb": "Фев (Feb)",
    "Mar": "Мар (Mar)",
    "Apr": "Апр (Apr)",
    ...
    "Dec": "Дек (Dec)",
    "Total": "Итого (Total)",
    "Compatibility Matrix": "Матрица совместимости (Compatibility Matrix)",
    "Selected inverter": "Выбранный инвертор (Selected inverter)",
    "Selected panel": "Выбранная панель (Selected panel)",
    "Selected battery": "Выбранная АКБ (Selected battery)",
    "PV voltage/current": "Напряжение/ток PV (PV voltage/current)",
    "Battery compatibility": "Совместимость АКБ (Battery compatibility)",
    "Data completeness": "Полнота данных (Data completeness)",
    "Combined result": "Сводный результат (Combined result)",
    "Cold Voc vs max PV voltage": "Холодный Voc против max PV voltage (Cold Voc vs max PV voltage)",
    "Hot Vmp vs MPPT range": "Горячий Vmp против диапазона MPPT (Hot Vmp vs MPPT range)",
    "Panel current vs MPPT current": "Ток панели против тока MPPT (Panel current vs MPPT current)",
    "Battery protocol": "Протокол АКБ (Battery protocol)",
    "Required datasheet data": "Требуемые данные datasheet (Required datasheet data)",
    "Adjust panels per string or inverter": "Изменить панели в строке или инвертор (Adjust panels per string or inverter)",
    "Adjust panels per string": "Изменить панели в строке (Adjust panels per string)",
    "Adjust strings per MPPT or inverter": "Изменить строки на MPPT или инвертор (Adjust strings per MPPT or inverter)",
    "Verify CAN/RS485 settings": "Проверить настройки CAN/RS485 (Verify CAN/RS485 settings)",
    "Fill missing database fields": "Заполнить недостающие поля базы (Fill missing database fields)",
}


def translate_cell_text(value: object) -> object:
    if isinstance(value, str) and value in RU_TEXT:
...
def cell_xml(ref: str, value: object, formula: str | None, style_id: int | None) -> str:
    ...
    value = translate_cell_text(value)
...
def data_validation(cell: str, formula_range: str, title: str, prompt: str) -> str:
    title = str(translate_cell_text(title))
    prompt = str(translate_cell_text(prompt))
    return (
